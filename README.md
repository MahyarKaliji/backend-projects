# Backend Roadmap Projects

- [Unit Converter](https://github.com/MahyarKaliji/backend-projects/tree/main/unit-converter) - _Javascript (Node.js)_

Sample solution for the [unit-converter](https://roadmap.sh/projects/unit-converter) challenge from [roadmap.sh](https://roadmap.sh/).

---

- [Task Tracker CLI](https://github.com/MahyarKaliji/backend-projects/tree/main/task-tracker) - _Javascript (Node.js)_

Sample solution for the [task-tracker](https://roadmap.sh/projects/task-tracker) challenge from [roadmap.sh](https://roadmap.sh/).

---

- [Github User Activity](https://github.com/MahyarKaliji/backend-projects/tree/main/github-user-activity) - _Javascript (Node.js)_

Sample solution for the [github-user-activity](https://roadmap.sh/projects/github-user-activity) challenge from [roadmap.sh](https://roadmap.sh/).

---

- [Expense Tracker](https://github.com/MahyarKaliji/backend-projects/tree/main/expense-tracker) - _Javascript (Node.js)_

Sample solution for the [expense-tracker](https://roadmap.sh/projects/expense-tracker) challenge from [roadmap.sh](https://roadmap.sh/).

---

- [Number Guessing Game](https://github.com/MahyarKaliji/backend-projects/tree/main/number-guessing-game) - _Javascript (Node.js)_

Sample solution for the [number-guessing-game](https://roadmap.sh/projects/number-guessing-game) challenge from [roadmap.sh](https://roadmap.sh/).
